...
export const deleteTeamExperiment = async (experimentId: string, teamId:string) => {
  const user_token = await getValueForStore("user_token");
  if (!user_token) {
    alert("User must be signed in!");
    return [[], 0];
  ...
  try {
    const response = await axios({
      method: "post",
      url: baseUrl + "/delete-team-experiment",
      headers: {
        Authorization: `Bearer ${user_token}`, // Adiciona o token no cabeçalho
      },
      data: {
...
export const createTeamExperiment = async (name: string, teamId: string) => {
  const user_token = await getValueForStore("user_token");
  if (!user_token) {
    alert("User must be signed in!");
    return [[], 0];
  ...
  try {
    const response = await axios({
      method: "post",
      url: baseUrl + "/create-team-experiment",
      headers: {
        Authorization: `Bearer ${user_token}`, // Adiciona o token no cabeçalho
      },
      data: {
...
export const saveTeamExperiment = async (
  ...
  try {
    const response = await axios({
      method: "post",
      url: baseUrl + "/save-team-experiment",
      headers: {
        Authorization: `Bearer ${user_token}`, // Adiciona o token no cabeçalho
      },
      data: {
...
export const saveTeamNotes = async (
  experimentId: string,
  notes: string,
  teamId: string
) => {
